 ...
 */
export async function addToWatchlist(userId: number, movieId: number): Promise<WatchlistItem | null> {
  const endpoint = `/users/${userId}/watchlist/add/${movieId}`

  try {
    const response: Response = await apiClient(endpoint, {method: 'PUT'});
    const data = await response.json();
    return await data.association_object;
  } catch (error: unknown) {
    if (error instanceof Error)
